...
function Navbar() {
  ...
  return (
    <div className="backdrop-blue-md fixed top-0 z-[100] mx-auto flex h-[77px] w-full items-center justify-evenly bg-white">
      <div>
        <Logo />{' '}
      </div>
        ...
        {navLinks.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              href={item.path}
              className={`relative border-violet-500 px-4 py-2 text-xl text-[#5C5CA0] no-underline duration-300 ease-in lg:text-base ${isActive ? 'font-semibold' : 'font-normal'}`}
              onMouseOver={() => {
